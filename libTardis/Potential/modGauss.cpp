...
void fGaussHermitePolys(Col<double> *mX, Mat<double> *mP, int iN) {
    ...
    for(j=1; j<=iNX; j++) {
        mP(j,1) = pow(M_PI,-0.25);
        if(iN>0) {
            mP(j,2) = sqrt(2)*pow(M_PI,-0.25)*mX(j);
        }
    }
    ...
    for(n=1; n<iN; n++) {
        for(j=1; j<=iNX; j++) {
            mP(j,n+2) = sqrt(2.0/(n+1.0))*mX(j)*mP(j,n+1) - sqrt(n/(n+1.0))*mP(j,n);
        }
    }
...
void fGaussLaguerrePolys(Col<double> *mX, Mat<double> *mP, int iN, double dAlpha) {
    ...
    for(n=1; n<iN; n++) {
        for(j=1; j<=iNX; j++) {
            mP(j,n+2) = 1.0/(n+1)*((2*n+1+dAlpha-mX(j))*mP(j,n+1) - (n+dAlpha)*mP(j,n));
        }
    }
...
void fGaussNormalizedLaguerrePolys(Col<double> *mX, Mat<double> *mP, int iN, double dAlpha) {
    ...
    // Generate first two polynomials.
    for(j=1; j<=iNX; j++) {
        mP(j,1) = 1.0/exp(0.5*gamma(dAlpha+1));
        if(iN>0) {
            mP(j,2) = (-mX(j) + dAlpha + 1.0)/exp(0.5*gamma(dAlpha+2));
        }
    }

    // Generate the rest by recursion
    for(n=1; n<iN; n++) {
        double dC1 = sqrt((n+dAlpha+1)*(n+1));
        double dC2 = sqrt(n*(n+dAlpha));
        for(j=1; j<=iNX; j++) {
            mP(j,n+2) = (2*n+1+dAlpha-mX(j))*mP(j,n+1)/dC1 - (dC2/dC1)*mP(j,n);
        }
    }
