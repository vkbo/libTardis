...
class simple_matrix {
    ...
    simple_matrix() : data_(0), nrows_(0), ncols_(0), ld_(0), active_rows_(), active_cols_(), use_block_(false) { }
    ...
    simple_matrix(index_t nrows, index_t ncols, index_t ld = 0) : active_rows_(), active_cols_(), use_block_(false) {
    ...
    void resize(index_t nrows, index_t ncols, index_t ld = 0) {
        ...
        if (ld < nrows) ld_ = nrows_;
        else            ld_ = ld;

        if ( (index_t)data_.size() != (index_t)(ld_ * ncols_) ) data_.resize(ld_ * ncols_);
    }
    ...
    simple_matrix<T>& operator*=(const T factor) {
        for (index_t col = 1; col <= ncols_; ++col) {
            ...
            for (index_t k = 0; k < nrows_; ++k) *(first + k) *= factor;
        }
        return *this;
    }
