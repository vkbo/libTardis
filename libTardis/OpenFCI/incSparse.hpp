...
class SparseMatrix : public std::map<I, SparseVector<I, T> > {
    ...
    /// Default constructor
    SparseMatrix<I, T>() : std::map<I, SparseVector<I, T> >(), nrows(0), ncols(0), structure_computed(false) { }
    ...
    void setNrows(I n) {
        nrows = n;
        if ((nrows>0) && (ncols>0)) structure_computed = true;
    }
    ...
    I getNnz() {
        ...
        for (it=this->begin(); it!=this->end(); ++it) count += it->second.size();
    ...
    void saveTranspose(SparseMatrix<I, T>& dst) {
        ...
        for (it = this->begin(); it != this->end(); ++it) {
            ...
            for (it2 = it->second.begin(); it2 != it->second.end(); ++it2) dst[it2->first][it->first] = it2->second;
        }
...
class SparseMatrixCrs {
    ...
    double element(size_t row, size_t the_col) {
        ...
        for (size_t k = row_ptr[row]; k < row_ptr[row+1]; ++k) {
            if (col[k] == the_col) return data[k];
        }
...
template<class I, class T>
void SparseMatrix_mult_symm(SparseMatrix<I,T>& A, SparseMatrix<I,T>& B, SparseMatrix<I,T>& C) {
