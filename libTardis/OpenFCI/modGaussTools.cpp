...
void computeHermitePolys(dense_vector& x, dense_matrix& P, int N) {
    ...
    for (j=1; j<=nx; ++j) {
        P(j,1) = pow(M_PI,-0.25);
        if (N>0) P(j,2) = sqrt(2)*pow(M_PI,-0.25)*x(j);
    }
...
void computeLaguerrePolys(dense_vector& x, dense_matrix& P, int N, double alpha) {
    ...
    for (j=1; j<=nx; ++j) {
        P(j,1) = 1.0;
        if (N>0) P(j,2) = -x(j) + alpha + 1.0;
    }

    // Generate the rest by recursion
    for (n = 1; n<N; ++n) {
        for (j=1; j<=nx; ++j) P(j,n+2) = 1.0/(n+1)*((2*n+1+alpha-x(j))*P(j,n+1) - (n+alpha)*P(j,n));
    }
}
...
void computeNormalizedLaguerrePolys(dense_vector& x, dense_matrix& P, int N, double alpha) {
    ...
    for (j=1; j<=nx; ++j) {
        ...
        if (N>0) P(j,2) = (-x(j) + alpha + 1.0)/exp(0.5*gamma(alpha+2));
    }
...
void computeGenHalfGaussHermite(dense_vector& x, dense_vector& w, double gam, long int N) {
    ...
    for (int n = 0; n<=N; ++n) {
        J(n+1,n+1) = alpha(n+1);
        if (n>0) J(n+1,n) = sqrt(beta(n+1));
        if (n<N) J(n+1,n+2) = sqrt(beta(n+2));
    }
...
void computeGenHalfHermiteCoeffs(dense_vector& alpha, dense_vector& beta, double gam, int N) {
    ...
    for (iter = 1; iter <= maxiter; ++iter) {
        ...
        for (int j = 1; j <= N; ++j) {
            ...
            if (j>1) J(j,j-1) =-p1*sqr(p3);
            if (j<N) J(j,j+1) = -p2*sqr(p3);
...
void computeNormalizedGenHalfHermitePolys(dense_vector& x, dense_matrix& P, double gam, int N, bool diff) {
    ...
    for (j=1; j<=nx; ++j) P(j,1) = pow(0.5*exp(gamma((gam+1)/2)), -0.5);
    ...
    for (n = 0; n<N; ++n) {
        if (n>0) {
            for (j=1;j<=nx; ++j) {
                ...
                if (diff) P(j,K + n+2) = (a(n+1)*x(j) - b(n+1))*P(j,K + n+1) - c(n+1)*P(j,K+n) + a(n+1)*P(j,n+1);
            }
        } else {
            for (j=1;j<=nx; ++j) {
                P(j,n+2) = (a(n+1)*x(j) - b(n+1))*P(j,n+1);
                if (diff) P(j,K + n+2) = (a(n+1)*x(j) - b(n+1))*P(j,K + n+1) + a(n+1)*P(j,n+1);
            }
        }
    }
}
